...
namespace BepuPhysics.CollisionDetection.CollisionTasks
{
    ...
    public static class ManifoldCandidateHelper
    {
        ...
        /// <summary>
        /// Reduces a set of contact candidates and outputs only a set of feature ids.
        /// </summary>
        /// <param name="candidates">Pointer to the candidates to reduce</param>
        /// <param name="candidateDepths">Pointer to the depths of the candidates to reduce.</param>
        /// <param name="candidateCount">Number of candidates.</param>
        /// <param name="epsilonScale">Representative scale of the manifold for dynamic size tuning.</param>
        /// <param name="reducedFeatureIds">Pointer to the set of output feature ids found to be necessary.</param>
        /// <param name="reducedCount">Number of candidates found to be necessary.</param>
        public unsafe static void Reduce(ManifoldCandidateScalar* candidates, float* candidateDepths, int candidateCount, float epsilonScale, int* reducedFeatureIds, out int reducedCount)
        {
            if (candidateCount <= 4)
            {
                ...
                for (int i = 0; i < candidateCount; ++i)
                {
                    reducedFeatureIds[i] = candidates[i].FeatureId;
                }
                reducedCount = candidateCount;
                return;
            }
            ...
            var bestIndex0 = 0;
            //While depth is the dominant heuristic, extremity is used as a bias to keep initial contact selection a little more consistent in near-equal cases.
            var extremityScale = epsilonScale * 5e-3f;
            var extremityX = 0.7946897654f * extremityScale;
            var extremityY = 0.60701579614f * extremityScale;
            for (int i = 0; i < candidateCount; ++i)
            {
            ...
            var candidate0 = candidates[bestIndex0];
            reducedFeatureIds[0] = candidate0.FeatureId;
            RemoveCandidateAt(candidates, candidateDepths, bestIndex0, ref candidateCount);

            //Find the most distant point from the starting contact.
            var maximumDistanceSquared = -1f;
            var bestScore1 = float.MinValue;
            var bestIndex1 = 0;
            var distanceScoreScale = epsilonScale * epsilonScale * 1e-3f;
            for (int i = 0; i < candidateCount; ++i)
            {
                ref var candidate = ref candidates[i];
                var offsetX = candidate.X - candidate0.X;
                ...
                //Penalize speculative contacts; they are not as important in general.
                //distanceSquared = candidateDepths[i] < 0 ? 0.125f * distanceSquared : distanceSquared;
                //var candidateScore = candidateDepths[i] + distanceSquared * distanceScoreScale;
                var candidateScore = distanceSquared;

                if (distanceSquared > maximumDistanceSquared)
                    maximumDistanceSquared = distanceSquared;
                if (candidateScore > bestScore1)
                {
            ...
            var candidate1 = candidates[bestIndex1];
            reducedFeatureIds[1] = candidate1.FeatureId;
            RemoveCandidateAt(candidates, candidateDepths, bestIndex1, ref candidateCount);
            ...
            var minSignedArea = 0f;
            var maxSignedArea = 0f;
            //var areaScoreScale = epsilonScale * epsilonScale * 1e-3f;
            //var bestScore2 = float.MaxValue;
            //var bestScore3 = float.MinValue;
            var bestIndex2 = 0;
            var bestIndex3 = 0;
            for (int i = 0; i < candidateCount; ++i)
            {
                ...
                var candidateOffsetY = candidate.Y - candidate0.Y;
                var signedArea = candidateOffsetX * edgeOffsetY - candidateOffsetY * edgeOffsetX;
                //var scaledArea = signedArea * areaScoreScale;
                //var minCandidateScore = scaledArea - candidateDepths[i];
                //var maxCandidateScore = scaledArea + candidateDepths[i];
                //if (bestScore2 > minCandidateScore)
                //{
                //    bestIndex2 = i;
                //    bestScore2 = minCandidateScore;
                //}
                //if (bestScore3 < maxCandidateScore)
                //{
                //    bestIndex3 = i;
                //    bestScore3 = maxCandidateScore;
                //}
            ...
            //reducedCount = 2;
            //reducedFeatureIds[reducedCount++] = candidates[bestIndex2].FeatureId;
            //if (bestIndex2 != bestIndex3)
            //    reducedFeatureIds[reducedCount++] = candidates[bestIndex3].FeatureId;
            var areaEpsilon = epsilonScale * epsilonScale * 1e-3f;
            reducedCount = 2;
            if (minSignedArea < -areaEpsilon)
            {
                reducedFeatureIds[reducedCount++] = candidates[bestIndex2].FeatureId;
            }

            if (maxSignedArea > areaEpsilon)
            {
                reducedFeatureIds[reducedCount++] = candidates[bestIndex3].FeatureId;
            }
        }
